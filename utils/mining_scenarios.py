...
def mine_solo_scenarios(tags_dict:dict)->dict:
    ...
    actors_static_element_intersection = tags_dict['actors_static_element_intersection']
    solo_scenarios = {}
    band_scenarios = {}
    for (actor_type,actors) in actors_list.items():
        ...
        for agent in actors:
            ...
            turning_points = __computing_turning_point(agent_lo_act,valid_start,valid_end)
            solo_scenarios[actor_type][agent_key]['lo']=__summarizing_events(agent_lo_act,turning_points,valid_start,valid_end,'lo',lo_act_dict)        
            agent_la_act = agent_activity['la_act']
            turning_points = __computing_turning_point(agent_la_act,int(valid_start),int(valid_end))
            solo_scenarios[actor_type][agent_key]['la']=__summarizing_events(agent_la_act,turning_points,valid_start,valid_end,'la',la_act_dict)
            ...
            for (element_type,element) in agent_elements_relation.items():
                ...
                if sum(relation[valid_start:valid_end+1])== -(valid_end-valid_start+1):
                    ...
                    solo_scenarios[actor_type][agent_key]['static'][element_type] = __summarizing_events(relation,turning_points,valid_start,valid_end,element_type,road_relation_dict)
                    if element_type.startswith("controlled_lane"):
                        turning_points = __computing_turning_point(element['light_state'],valid_start,valid_end)
                        solo_scenarios[actor_type][agent_key]['static'][element_type]['light_state'] = __summarizing_events(element['light_state'],turning_points,valid_start,valid_end,'light',light_state_dict)
